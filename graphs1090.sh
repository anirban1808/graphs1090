...
rrdtool graph \
    ...
    \
    "TEXTALIGN:center" \
    "DEF:fr24_latency=/run/collectd/localhost/network_monitor-fr24/gauge-latency_fr24.rrd:value:AVERAGE" \
    "DEF:fa_latency=/run/collectd/localhost/network_monitor-fa/gauge-latency_fa.rrd:value:AVERAGE" \
    "DEF:ssid_raw=/run/collectd/localhost/network_monitor-ssid/gauge-ssid_status.rrd:value:MAX" \
    \
    "CDEF:ssid_smoothed=ssid_raw,UN,PREV,ssid_raw,IF" \
    "CDEF:ssid_filled=ssid_smoothed,UN,PREV,ssid_smoothed,IF" \
    "CDEF:ssid0=ssid_filled,0,EQ,INF,UNKN,IF" \
    "CDEF:ssid1=ssid_filled,1,EQ,INF,UNKN,IF" \
    "CDEF:ssid2=ssid_filled,2,EQ,INF,UNKN,IF" \
    "AREA:ssid0#CFFFA0::STACK:skipscale" \
    "AREA:ssid1#CE96D2::STACK:skipscale" \
    "AREA:ssid2#84FFFF::STACK:skipscale" \
    \
    "CDEF:fr24_alert=fr24_latency,1000,GT,fr24_latency,UNKN,IF" \
    "CDEF:fa_alert=fa_latency,1000,GT,fa_latency,UNKN,IF" \
    \
    "VDEF:fr24_avg=fr24_latency,AVERAGE" \
    "VDEF:fr24_last=fr24_latency,LAST" \
    "VDEF:fa_avg=fa_latency,AVERAGE" \
    "VDEF:fa_last=fa_latency,LAST" \
    \
    "GPRINT:fr24_avg:Flightradar24 \\: Avg %5.1lf ms" \
    "GPRINT:fr24_last: / Last %5.1lf ms\\c" \
    "GPRINT:fa_avg:FlightAware   \\: Avg %5.1lf ms" \
    "GPRINT:fa_last: / Last %5.1lf ms\\c" \
    \
    "LINE1:fr24_latency#BE8264:Latency\\:Flightradar24" \
    "LINE1:fr24_alert#BE1EC8:Alert (>1s)                  " \
    "LINE1:fa_latency#00417D:Latency\\:FlightAware" \
    "LINE1:fa_alert#FF0000:Alert (>1s)\\c" \
    "COMMENT:                             " \
    "COMMENT:Connected SSID\\: " \
    "LINE1:0#CFFFA0:[Tenda_Misshka] " \
    "LINE1:0#CE96D2:[MisshkaTel] " \
    "LINE1:0#84FFFF:[SEN147w]\\n" \
    \
    --watermark "Drawn: $(date)"
    mv "${out_png}.tmp" "$out_png"
}
...
system_graphs() {
	...
	latency_graph ${DOCUMENTROOT}/system-$2-latency-$4.png ${DB}/$2/network_monitor "Latency to FR24 & FA" "$4" "$2"
	if [[ $farenheit == 1 ]]
	then
